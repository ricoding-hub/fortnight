...
export function resolveDebtGoal(goal: Goal, { creditDebt, disposable }: DebtGoalInputs): Goal | null {
  // `?.` y no `.`: el tipo promete el arreglo, pero quien lo llena es una
  // consulta a otra tabla. Si esa consulta falla o la meta llega por un camino
  // optimista, aquí llega undefined y leer `.length` revienta al dibujar.
  if (!goal.is_debt || (goal.linked_account_ids?.length ?? 0) > 0) {
